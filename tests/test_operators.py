...
def test_symmetric() -> None:
    ...
    """
    samples = [-5.0, -1.5, -0.0, 0.0, 0.5, 2.0, 7.5]
    for x in samples:
        for y in samples:
...
def test_distribute() -> None:
    ...
    """
    samples = [-3.0, -1.0, 0.0, 0.5, 2.0, 4.0]
    for z in samples:
        for x in samples:
            for y in samples:
                left = mul(z, add(x, y))
...
def test_other() -> None:
    ...
    samples = [-10.0, -2.5, -1.0, 0.0, 1.0, 3.3, 9.0]
    for a in samples:
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    pairwise = addLists(ls1, ls2)
    right = minitorch.operators.sum(pairwise)
    assert_close(left, right)
